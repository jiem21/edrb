...
class DRB_Generate extends dbh
{
	...
	public function template($drb_number)
	{
		...
		// Query for Affected Lots
			$lots = pg_query($this->con,"SELECT * FROM tbl_drb_affected_lots where rfc_no = '$rfc'");
			$count_total = pg_num_rows($lots);
	...
	public function generate_report_data($column_name,$date1,$date2)
	{
		...
		$columnBlock = 3;
		// DRB BY Block
		$generate_month_block = pg_query($this->con,"SELECT distinct DATE_PART('month',occur_date) ,occur_month, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE $column_name between '$date1' AND '$date2' ORDER BY DATE_PART('month',occur_date), occur_year ASC");
		...
		while ($get_month_block = pg_fetch_array($generate_month_block)) {
			...
			while ($get_block_data = pg_fetch_array($get_block)) {
				...
				$get_data = pg_query($this->con,"SELECT count(b.blocks) data_count, b.blocks,a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
					FROM tbl_blocks b
					left Join  tbl_drb_tracking_ledger a on a.block = b.blocks
					where a.block = '$block' AND a.occur_month = '$use_month' AND a.$column_name between '$date1' AND '$date2' GROUP BY b.blocks, a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year;");
			...
			$actual = pg_query($this->con,"SELECT count(a.occur_month) data_count, a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
				FROM tbl_blocks b
				left Join  tbl_drb_tracking_ledger a on a.block = b.blocks
				where a.occur_month = '$use_month' AND a.$column_name between '$date1' AND '$date2' GROUP BY a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year");
		...
		// DRB by 5M1E
		$generate_month_5M1E = pg_query($this->con,"SELECT distinct DATE_PART('month',occur_date) ,occur_month, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE $column_name between '$date1' AND '$date2' ORDER BY DATE_PART('month',occur_date), occur_year ASC");
		...
		while ($get_month_5M1E = pg_fetch_array($generate_month_5M1E)) {
			...
			while ($get_m5e1_data = pg_fetch_array($get_m5e1)) {
				...
				$get_data = pg_query($this->con,"SELECT count(b.method_type) data_count, b.method_type,a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
					FROM tbl_m5e1 b
					right Join  tbl_drb_tracking_ledger a on a.m5e1 = b.method_type
					where a.m5e1 = '$m5e1' AND a.occur_month = '$use_month'  AND a.occur_date between '$date1' AND '$date2'  GROUP BY b.method_type, a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year;");
				$count_blocks = pg_num_rows($get_data);
				if ($count_blocks >= 1) {
					while ($get_total = pg_fetch_array($get_data)) {
		...
		// DRB by RANK LEVEL
		$generate_month_rank = pg_query($this->con,"SELECT distinct DATE_PART('month',occur_date) ,occur_month, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE $column_name between '$date1' AND '$date2' ORDER BY DATE_PART('month',occur_date), occur_year ASC");
		...
		while ($get_month_rank = pg_fetch_array($generate_month_rank)) {
			...
			while ($get_rank_data = pg_fetch_array($get_rank)) {
				...
				$get_data = pg_query($this->con,"SELECT count(b.rank) data_count, b.rank,a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
					FROM tbl_closing_approval b
					right Join  tbl_drb_tracking_ledger a on a.rank = b.rank
					where a.rank = '$rank' AND a.occur_month = '$use_month'  AND a.$column_name between '$date1' AND '$date2'  GROUP BY b.rank, a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year;");
				$count_blocks = pg_num_rows($get_data);
				if ($count_blocks >= 1) {
					while ($get_total = pg_fetch_array($get_data)) {
		...
		// DRB by Closure
		$generate_month_closure = pg_query($this->con,"SELECT distinct DATE_PART('month',occur_date) ,occur_month, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE $column_name between '$date1' AND '$date2' ORDER BY DATE_PART('month',occur_date), occur_year ASC");
		...
		while ($get_month_closure = pg_fetch_array($generate_month_closure)) {
			...
			$get_open = pg_query($this->con,"SELECT * FROM tbl_drb_tracking_ledger WHERE drb_status = 1 and occur_month = '$use_month'");
			$get_close = pg_query($this->con,"SELECT * FROM tbl_drb_tracking_ledger WHERE drb_status = 0 and occur_month = '$use_month'");
		...
		// DRB by Reccurence
		$generate_month_reccurence = pg_query($this->con,"SELECT distinct DATE_PART('month',occur_date) ,occur_month, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE $column_name between '$date1' AND '$date2' ORDER BY DATE_PART('month',occur_date), occur_year ASC");
		...
		while ($get_month_reccurence = pg_fetch_array($generate_month_reccurence)) {
			...
			while ($get_m5e1_data = pg_fetch_array($get_m5e1)) {
				...
				$get_data = pg_query($this->con,"SELECT count(b.method_type) data_count, b.method_type,a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
					FROM tbl_m5e1 b
					right Join  tbl_drb_tracking_ledger a on a.m5e1 = b.method_type
					where a.m5e1 = '$m5e1' AND a.occur_month = '$use_month' AND lower(a.issue_type) = lower('Recurrence') AND a.occur_date between '$date1' AND '$date2'  GROUP BY b.method_type, a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year;");
				$count_blocks = pg_num_rows($get_data);
				if ($count_blocks >= 1) {
					while ($get_total = pg_fetch_array($get_data)) {
				...
				$prepare_query1 = pg_query($this->con,"SELECT * FROM tbl_drb_tracking_ledger where issue_type = 'Recurrence' and occur_month = '$use_month';");
				$prepare_query2 = pg_query($this->con,"SELECT * FROM tbl_drb_tracking_ledger where occur_month = '$use_month';");
		...
		while ($get_month_block_lot_out = pg_fetch_array($generate_month_block)) {
			...
			while ($get_block_data = pg_fetch_array($get_block)) {
				...
				$get_data = pg_query($this->con,"SELECT sum(a.lot_out) data_count, b.blocks,a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
					FROM tbl_blocks b
					left Join  tbl_drb_tracking_ledger a on a.block = b.blocks
					where a.block = '$block' AND a.occur_month = '$use_month' AND a.$column_name between '$date1' AND '$date2' GROUP BY b.blocks, a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year;");
				$count_blocks = pg_num_rows($get_data);
			...
			$actual = pg_query($this->con,"SELECT sum(a.lot_out) data_count, a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
				FROM tbl_blocks b
				left Join  tbl_drb_tracking_ledger a on a.block = b.blocks
				where a.occur_month = '$use_month' AND a.$column_name between '$date1' AND '$date2' GROUP BY a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year");
			while ($actual_count = pg_fetch_array($actual)) {
		...
		// DRB Lot out by 5M1E
		$generate_month_5M1E_lot_out = pg_query($this->con,"SELECT distinct DATE_PART('month',occur_date) ,occur_month, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE $column_name between '$date1' AND '$date2' ORDER BY DATE_PART('month',occur_date), occur_year ASC");
		...
		while ($get_month_5M1E_lot_out = pg_fetch_array($generate_month_5M1E_lot_out)) {
			...
			while ($get_m5e1_data = pg_fetch_array($get_m5e1)) {
				...
				$get_data = pg_query($this->con,"SELECT sum(a.lot_out) data_count, b.method_type,a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
					FROM tbl_m5e1 b
					right Join  tbl_drb_tracking_ledger a on a.m5e1 = b.method_type
					where a.m5e1 = '$m5e1' AND a.occur_month = '$use_month'  AND a.occur_date between '$date1' AND '$date2'  GROUP BY b.method_type, a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year;");
				$count_blocks = pg_num_rows($get_data);
		...
		// Lot out/incident
		$generate_month_incident = pg_query($this->con,"SELECT distinct DATE_PART('month',occur_date) ,occur_month, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE $column_name between '$date1' AND '$date2' ORDER BY DATE_PART('month',occur_date), occur_year ASC");
		...
		while ($get_month_incident = pg_fetch_array($generate_month_incident)) {
			...
			// $rowCountIncident = 92;
				// Get lot out count
				$get_lotout = pg_query($this->con,"SELECT sum(lot_out) data_count, occur_month, DATE_PART('month',occur_date) occur_months, DATE_PART('year',occur_date) occur_year
					FROM tbl_drb_tracking_ledger
					where occur_month = '$use_month' AND occur_date between '$date1' AND '$date2'  GROUP BY occur_month, DATE_PART('month',occur_date), DATE_PART('year',occur_date) order by occur_months, occur_year;");
				...
				$get_incident = pg_query($this->con,"SELECT count(drb_number) issue_count, occur_month, DATE_PART('month',occur_date) occur_months, DATE_PART('year',occur_date) occur_year
					FROM tbl_drb_tracking_ledger
					where occur_month = '$use_month' AND occur_date between '$date1' AND '$date2'  GROUP BY occur_month, DATE_PART('month',occur_date), DATE_PART('year',occur_date) order by occur_months, occur_year;");
				...
				// Get percentage
				$get_lotout_no = pg_query($this->con,"SELECT sum(lot_out) data_count, occur_month, DATE_PART('month',occur_date) occur_months, DATE_PART('year',occur_date) occur_year
					FROM tbl_drb_tracking_ledger
					where occur_month = '$use_month' AND occur_date between '$date1' AND '$date2'  GROUP BY occur_month, DATE_PART('month',occur_date), DATE_PART('year',occur_date) order by occur_months, occur_year;");
				$lotout_no = pg_fetch_row($get_lotout_no);
				$get_incident_no = pg_query($this->con,"SELECT count(drb_number) issue_count, occur_month, DATE_PART('month',occur_date) occur_months, DATE_PART('year',occur_date) occur_year
					FROM tbl_drb_tracking_ledger
					where occur_month = '$use_month' AND occur_date between '$date1' AND '$date2'  GROUP BY occur_month, DATE_PART('month',occur_date), DATE_PART('year',occur_date) order by occur_months, occur_year;");
		...
		while ($get_month_incident = pg_fetch_array($generate_month_incident)) {
			...
			// $rowCountIncident = 92;
				// Get lot out count
				$get_affected = pg_query($this->con,"SELECT sum(affected_lots::integer) data_count, occur_month, DATE_PART('month',occur_date) occur_months, DATE_PART('year',occur_date) occur_year
					FROM tbl_drb_tracking_ledger
					where occur_month = '$use_month' AND occur_date between '$date1' AND '$date2'  GROUP BY occur_month, DATE_PART('month',occur_date), DATE_PART('year',occur_date) order by occur_months, occur_year;");
				...
				// Get percentage
				$get_affected_no = pg_query($this->con,"SELECT sum(affected_lots::integer) data_count, occur_month, DATE_PART('month',occur_date) occur_months, DATE_PART('year',occur_date) occur_year
					FROM tbl_drb_tracking_ledger
					where occur_month = '$use_month' AND occur_date between '$date1' AND '$date2'  GROUP BY occur_month, DATE_PART('month',occur_date), DATE_PART('year',occur_date) order by occur_months, occur_year;");
				...
				$incident = $incident_no['0'];
				$percentage = ($total_affected/$incident)*100;
				$percentage = $percentage."%";
		...
		$generate_month_MR = pg_query($this->con,"SELECT distinct DATE_PART('month',occur_date) ,occur_month, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE $column_name between '$date1' AND '$date2' ORDER BY DATE_PART('month',occur_date), occur_year ASC");
		...
		while ($get_month_MR = pg_fetch_array($generate_month_MR)) {
			...
			while ($get_MR_data = pg_fetch_array($get_blocks)) {
				...
				$get_data = pg_query($this->con,"SELECT count(b.blocks) data_count, b.blocks,a.occur_month, DATE_PART('month',a.occur_date) occur_months, DATE_PART('year',a.occur_date) occur_year
					...
					where a.block = '$block' AND a.occur_month = '$use_month' AND a.m5e1 = 'Man' AND a.occur_date between '$date1' AND '$date2'  GROUP BY b.blocks, a.occur_month, DATE_PART('month',a.occur_date), DATE_PART('year',a.occur_date) order by occur_months, occur_year;");
				$count_blocks = pg_num_rows($get_data);
				if ($count_blocks >= 1) {
					while ($get_total = pg_fetch_array($get_data)) {
