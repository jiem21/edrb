...
<script>
	...
	$generate_label2 = pg_query($db->con,"SELECT distinct occur_month,DATE_PART('month',occur_date) occur_mnth, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE occur_date between '$FY1' AND '$FY2' ORDER BY occur_year, occur_mnth DESC");
	...
	var DRB_BY_BLOCK = {
		...
		while ($blocks = pg_fetch_array($generate_block)) {
			...
			$generate_month = pg_query($db->con,"SELECT distinct DATE_PART('year',occur_date),DATE_PART('month',occur_date) occur_mnth ,occur_month FROM tbl_drb_tracking_ledger WHERE occur_date between '$FY1' AND '$FY2' ORDER BY DATE_PART('year',occur_date) ,DATE_PART('month',occur_date) DESC");
			?>

			{
				...
				while ($get_month = pg_fetch_array($generate_month)) {
					...
					$get_data = pg_query($db->con,"SELECT count(b.blocks) data_count, b.blocks,a.occur_month, DATE_PART('year',a.occur_date) occur_year
						FROM tbl_blocks b
						left Join  tbl_drb_tracking_ledger a on a.block = b.blocks
						where a.block = '$get_block' AND a.occur_month = '$use_month' AND a.occur_date between '$FY1' AND '$FY2' GROUP BY b.blocks, a.occur_month, DATE_PART('year',a.occur_date) order by occur_year;");
	...
	var rank_level = {
		...
		while ($rank = pg_fetch_array($generate_rank)) {
...

			{
				...
				while ($get_month = pg_fetch_array($generate_month)) {
					$use_month = $get_month['occur_month'];
					$get_data = pg_query($db->con,"SELECT count(b.rank) data_count, b.rank,a.occur_month, DATE_PART('year',a.occur_date) occur_year
						FROM tbl_closing_approval b
						left Join  tbl_drb_tracking_ledger a on a.rank = b.rank
						where a.rank = '$get_rank' AND a.occur_month = '$use_month'  AND a.occur_date between '$FY1' AND '$FY2'  GROUP BY b.rank, a.occur_month, DATE_PART('year',a.occur_date) order by occur_year;");
					$count_blocks = pg_num_rows($get_data);
					if ($count_blocks >= 1) {
						while ($get_total = pg_fetch_array($get_data)) {
							echo "'".$get_total['data_count']."',";
	...
	$generate_label3 = pg_query($db->con,"SELECT distinct occur_month,DATE_PART('month',occur_date) occur_mnth, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE occur_date between '$FY1' AND '$FY2' ORDER BY occur_year,occur_mnth DESC");

	$generate_label4 = pg_query($db->con,"SELECT distinct occur_month,DATE_PART('month',occur_date) occur_mnth, DATE_PART('year',occur_date) occur_year FROM tbl_drb_tracking_ledger WHERE occur_date between '$FY1' AND '$FY2' ORDER BY occur_year,occur_mnth DESC");
	?>
	...
	var closure = {
		...
		?>
		{
			...
			while ($get_data4 = pg_fetch_array($generate_label4)) {
				...
				$get_open = pg_query($db->con,"SELECT * FROM tbl_drb_tracking_ledger WHERE occur_date between '$FY1' AND '$FY2' AND drb_status = 1 and occur_month = '$use_month'");
				$get_close = pg_query($db->con,"SELECT * FROM tbl_drb_tracking_ledger WHERE occur_date between '$FY1' AND '$FY2' AND drb_status = 0 and occur_month = '$use_month'");
	...
	var DRB_Recurrence = {
		...
		datasets: [
		{
			...
			while ($month2 = pg_fetch_array($generate_label2)) {
				...
				$prepare_query1 = pg_query($db->con,"SELECT * FROM tbl_drb_tracking_ledger where occur_date between '$FY1' AND '$FY2' AND issue_type = 'Recurrence' and occur_month = '$occur_month';");
				$prepare_query2 = pg_query($db->con,"SELECT * FROM tbl_drb_tracking_ledger where occur_date between '$FY1' AND '$FY2' AND occur_month = '$occur_month';");
...

			{
				...
				while ($get_month = pg_fetch_array($generate_month)) {
					...
					$get_data = pg_query($db->con,"SELECT count(b.method_type) data_count, b.method_type,a.occur_month, DATE_PART('year',a.occur_date) occur_year
						FROM tbl_m5e1 b
						right Join  tbl_drb_tracking_ledger a on a.m5e1 = b.method_type
						where a.m5e1 = '$get_m5e1' AND a.occur_month = '$use_month' AND lower(a.issue_type) = lower('Recurrence') AND a.occur_date between '$FY1' AND '$FY2'  GROUP BY b.method_type, a.occur_month, DATE_PART('year',a.occur_date) order by occur_year;");
					$count_blocks = pg_num_rows($get_data);
					if ($count_blocks >= 1) {
						while ($get_total = pg_fetch_array($get_data)) {
							echo "'".$get_total['data_count']."',";
...

			{
				...
				while ($get_month = pg_fetch_array($generate_month)) {
					...
					$get_data = pg_query($db->con,"SELECT count(b.blocks) data_count, b.blocks,a.occur_month, DATE_PART('year',a.occur_date) occur_year
						FROM tbl_blocks b
						left Join  tbl_drb_tracking_ledger a on a.block = b.blocks
						where a.block = '$get_block' AND a.occur_month = '$use_month' AND a.m5e1 = 'Man' AND a.occur_date between '$FY1' AND '$FY2' GROUP BY b.blocks, a.occur_month, DATE_PART('year',a.occur_date) order by occur_year;");
					$count_blocks = pg_num_rows($get_data);
					if ($count_blocks >= 1) {
						while ($get_total = pg_fetch_array($get_data)) {
							echo "'".$get_total['data_count']."',";
